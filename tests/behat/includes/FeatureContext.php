...
namespace WordPress\Plugin_Check_Internal\Behat_Utils;
...
final class FeatureContext extends WP_CLI_FeatureContext {
	...
	/**
	 * @Given a WP install(ation) with the Plugin Check Internal (plugin)
	 */
	public function given_a_wp_installation_with_plugin_check_internal() {
		$this->install_wp();

		// Install latest Plugin Check.
		$this->proc( 'wp plugin install --activate https://github.com/WordPress/plugin-check/archive/refs/heads/trunk.zip' )->run_check();
		...
		$this->proc( "ln -s {$project_dir} {$plugin_dir}/plugin-check-internal" )->run_check();

		// Activate the plugin.
		$this->proc( 'wp plugin activate plugin-check-internal' )->run_check();

		// Install composer dependencies of Plugin Check.
		$this->proc( "composer install --working-dir={$plugin_dir}/plugin-check" )->run_check();
	}
